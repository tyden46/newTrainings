---
title: "Data mining and genomic plots"
...
### Linear combinations
Examine linear combinations of the genes to find a better distinction between
the two cancer stages. Linear discriminant analysis.

#### Standardize all the counts, fit a classification model

`scale` function standardizes 

```{r}
tr.s.sd <- data.frame(Patient_id=tr.s[,1], scale(tr.s[,2:63]), Cinical_Status=tr.s[,64])
# library(MASS)
tr.lda <- MASS::lda(Cinical_Status~., data=tr.s.sd[,-1])
...
20,534 genes and 475 patients is high dimension, low sample size
